...
async def patch_device_info(class_: str) -> None:
    ...
    * ``state``: the clean-info answer is a constant ``idle`` regardless of
      ...
      ``MowerStateRefresh``.
    * ``stats.clean``: ``GetStats`` drops ``mowedArea``, so it is swapped for
      ``GetStatsMower``.
    * ``life_span.get``: ``GetLifeSpan`` raises on ``uwbCell`` entries, so it is
      swapped for ``GetLifeSpanMower``.
    * mower-specific refreshes: ``getProtectState``, ``getRainDelay``,
      ``getStats``, and ``getLifeSpan`` are registered for the events the
      library does not provide capabilities for. A1600 area parameters and
      area names are registered here too, but only for the model for which
    ...
    **Must be called before ``ApiClient.get_devices()``.** That method calls
    ...
    patched = replace(
        ...
        # library's own and are carried through by replace().
        life_span=replace(
    ...
    # to hand dataclasses.replace. Put their refresh commands into the same
    # private event mapping that Capabilities.__post_init__ builds from its
    # dataclass fields. This is intentionally kept in this patch boundary: the
    # rest of the integration must not touch deebot-client internals.
    events = {
        **patched._events,
    ...
    if class_ in AREA_PARAMETER_CLASSES:
        # getAreaParameter and getAreaSet are two independent requests. They
        # publish separate events so a successful name response cannot replace
        # a valid parameter snapshot with an empty one (or vice versa).
