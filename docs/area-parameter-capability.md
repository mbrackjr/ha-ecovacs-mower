...
The mower also sends `onAreaParameter` unsolicited — observed arriving roughly 150ms after a `setAreaParameter` write, well before an explicit `getAreaParameter` refresh would land. It carries the identical `areaParameters` list as `getAreaParameter`'s answer and is parsed by the same code (`apply_area_parameters`), so either path updates the same snapshot. `setAreaParameter` itself does not support deebot-client's usual MQTT p2p echo handling (the `SetCommand`/`get_command` pairing other settings use), which is why this relies on registering the push directly instead.
