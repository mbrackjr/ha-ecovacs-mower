...
- `area_sensors._PendingAreaWrite` closes what remains: while a write is unconfirmed, the next write to the same area merges against the values just sent rather than the last confirmed snapshot. This matters even for writes dispatched at the same time — Home Assistant runs each `number.set_value` call as its own asyncio task, so two overlapping writes to the same area are a real, not just theoretical, case. `EcovacsAreaNumber.async_set_native_value` records the pending values *before* awaiting the mower's confirmation, not after, specifically so this holds under that overlap: everything from reading the current values through recording the merged result is synchronous, so the whole sequence is atomic with respect to any other coroutine. Recording it after the `await` instead would reopen the same race for two writes in flight at once, even though it looks correct for two writes issued one after the other — the mistake an earlier version of this fix made, caught in review. This does not depend on the push arriving — it is what protects a mower or firmware that never sends `onAreaParameter`.
