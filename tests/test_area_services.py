...
from custom_components.ecovacs_mower.deebot_patch.areas import A1600_PROFILE, MowerArea


@pytest.mark.parametrize("height, wire", [(3.0, 7), (6.0, 4), (9.0, 1)])
...
@pytest.mark.parametrize("speed, wire", [(0.40, 7), (0.55, 4), (0.70, 1)])
...
async def test_set_area_parameters_sends_all_fields_and_keeps_omitted_values(
    ...
    current = MowerArea(area_id="2", mow_height_level=4, cut_mode=5, obstacle_height=2, angle=214)
    ...
    call = Mock()
    call.data = {"device_id": "ha-device", "area_id": 2, "speed_mps": "0.60"}
